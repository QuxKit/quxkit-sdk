// Build config. The root entry and the `./pg` adapter — ESM + CJS +
// declarations for each.
//
// tsup (esbuild) rather than `tsc`, for the same structural reason as
// tenant-kit and billing-kit: `moduleResolution: bundler` lets src/ import
// extensionless while `tsc` would copy those specifiers through verbatim into
// ESM that Node's resolver rejects. esbuild resolves and rewrites them.
